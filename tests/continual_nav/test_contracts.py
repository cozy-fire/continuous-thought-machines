...
from tasks.continual_nav.contracts import (CTMState, DualState, FisherSequenceBatch,
                                         PhaseKey, PolicyOutput, Transition)


class ContractTests(unittest.TestCase):
    def test_explicit_policy_state_tags(self):
        a = CTMState(torch.zeros(2, 512, 20), torch.zeros(2, 512, 20))
        b = CTMState(torch.ones(2, 512, 20), torch.ones(2, 512, 20))
    ...
    def test_reward_free_records(self):
        for cls in (Transition, FisherSequenceBatch):
    ...
    def test_phase_identity(self):
        ...
        self.assertEqual(str(PhaseKey("ta", "maze_medium", 0, 1, phase="W", subphase="collect")),
                         "ta/v0/maze_medium/r1/W/collect")
        ...
        invalid = [dict(family="init", task="maze_medium"), dict(family="pnc", task="maze_medium", visit=0, phase="X"),
                   dict(family="ta", task="maze_medium", visit=0, phase="W"),
                   dict(family="seq", task="fourrooms", visit=-1, phase="P"),
                   dict(family="seq", task="fourrooms", visit=0, phase="P", subphase="fit")]
